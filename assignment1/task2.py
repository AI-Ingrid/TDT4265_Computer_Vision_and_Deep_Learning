...
def calculate_accuracy(X: np.ndarray, targets: np.ndarray, model: BinaryModel) -> float:
    ...
    # Accuracy = (True 1 + True 0) / batch size
    pred = np.round(model.forward(X))
    sum_correct_pred = np.sum(pred == targets)
    accuracy = sum_correct_pred/X.shape[0]
    return accuracy
...
class LogisticTrainer(BaseTrainer):
    ...
    def train_step(self, X_batch: np.ndarray, Y_batch: np.ndarray):
        ...
        for node, value in enumerate(self.model.w):                             # TODO: Avoid this loop
